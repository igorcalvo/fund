...
def drop_not_date(df: pd.DataFrame, year) -> pd.DataFrame:
    df = epd.where_column_equals(df, 'DT_INI_EXERC', f'{year}-01-01')
    return df
...
def calculate_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    account_index = epd.get_column_index(df, 'VL_CONTA')
